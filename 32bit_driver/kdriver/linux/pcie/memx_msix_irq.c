...
s32 memx_init_msix_irq(struct memx_pcie_dev *memx_dev)
{
	...
	if(memx_dev->msix) {
		...
		if (irq_request_or_err < 8) {
			pci_free_irq_vectors(memx_dev->pDev);

			pr_info("memryx: msix interrupt number %d not enough try alloc msi interrupt\n", irq_request_or_err);

			irq_request_or_err = pci_alloc_irq_vectors(memx_dev->pDev, 1, 32, PCI_IRQ_MSI);

			if (irq_request_or_err <= 0) {
				pr_err("memryx: init_msi/msix irq: fail to call pci_alloc_irq_vectors(%d)\n", irq_request_or_err);
				return -1;
			} else {
