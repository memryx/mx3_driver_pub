...
s32 memx_init_msix_irq(struct memx_pcie_dev *memx_dev)
{
	...
	if (irq_request_or_err <= 0) {
		pr_err("memryx: init_msix_irq: fail to call pci_alloc_irq_vectors(%d)\n", irq_request_or_err);
...
static s32 memx_enable_msix(struct memx_pcie_dev *memx_dev)
{
	...
	if (!memx_dev || !memx_dev->pDev) {
		pr_err("memryx: enable msix: invalid memx_dev\n");
		return -ENODEV;
	}
	...
	if (pci_read_config_dword(memx_dev->pDev, pcie_msix_cap_offset, &msix_capability_doward0) != 0) {
		pr_err("memryx: enable msix: Read MSIx Capability Dword0 from Configuration Space Failed!\n");
		res = -1;
		goto err;
	}
	...
	if (pci_write_config_dword(memx_dev->pDev, pcie_msix_cap_offset, ((msix_control << 16) | (msix_capability_doward0 & 0x0000FFFF))) != 0) {
		pr_err("memryx: enable msix: Write Back MSIx control Field Failed!\n");
		res = -2;
		goto err;
	}
#ifdef DEBUG
	pr_info("memryx: enable msix: Success!\n");
#endif
err:
...
static s32 memx_disable_msix(struct memx_pcie_dev *memx_dev)
{
	...
	if (!memx_dev || !memx_dev->pDev) {
		pr_err("memryx: disable msix: invalid memx_dev\n");
		return -ENODEV;
	}
	...
	pcie_msix_cap_offset = pci_find_capability(memx_dev->pDev, PCI_CAP_ID_MSIX);
	if (pci_read_config_dword(memx_dev->pDev, pcie_msix_cap_offset, &msix_capability_doward0) != 0) {
		pr_err("memryx: disable msix: Read MSIx Capability Dword0 from Configuration Space Failed!\n");
		res = -1;
		goto err;
	}
	...
	msix_control &= ~(BIT(15));
	if (pci_write_config_dword(memx_dev->pDev, pcie_msix_cap_offset, ((msix_control << 16) | (msix_capability_doward0 & 0x0000FFFF))) != 0) {
		pr_err("memryx: disable msix: Write Back MSIx control Field Failed!\n");
		res = -2;
		goto err;
	}
#ifdef DEBUG
	pr_info("memryx: disable msix: success!\n");
#endif
err:
	spin_unlock(&memx_dev->int_info.lock);
#ifdef DEBUG
...
{
	if (!memx_dev || !memx_dev->pDev) {
		pr_err("memryx: enable_device_msix_capability: invalid memx_dev\n");
		return -ENODEV;
	}
...
{
	if (!memx_dev || !memx_dev->pDev) {
		pr_err("memryx: disable_device_msix_capability: invalid memx_dev\n");
		return;
	}
	memx_disable_msix(memx_dev);
}
